...
| IC | 没有。单品种 0/1 时钟。 |
| 结论 | 最近这段折合成一年是亏的。对照事先写好的标准，这句话在最近这段不成立。结论留给你写。 |
...
| IC | 没有。单品种 0/1 时钟。 |
| 结论 | 最近这段折合成一年 −3.4%，回撤 −6.9% 深于两段趋势。两条事先写好的标准都被打中。结论留给你写。 |
...
| 分类 | 日历 alpha 声称 |
| 用哪种回测 | `event_backtest` · `000300.SH` · 日线 |
| 数据 | `mlbot data download-ashare`；A 股日历 `bear_2021` / `bull_924` / `chop_recent` |
| IC | 没有。日历 0/1 许可。 |
| 结论 | 三段折合成一年都是很小的正数；最近这段 +0.06%、Calmar 0.14。日历规律很弱。结论留给你写。 |
...
| 分类 | beta（山寨对 BTC） |
| 用哪种回测 | `event_backtest` · 2 小时 |
| 数据 | 山寨 tick 约从 2023-01 起；`bear_2022` **无样本** |
| IC | 没有。0/1 许可。 |
| 结论 | 最近这段折合成一年 −1.52%，回撤深于牛市那段。缺年的段不能只靠最近这段过关。结论留给你写。 |
...
| 分类 | 肥尾极薄，主形态小市值 beta |
| 用哪种回测 | `cohort_hold`（不是 `event_backtest`） |
| 数据 | 在市 + 退市日线；市值 = 当日成交额 / 换手率；`n` = 名字 × 入场季 |
| IC | 没有。问的是队列密度，不是每日分数。 |
| 结论 | 3 年十倍率两边都是 **0.11%**。股灾、2018 年相对折合成一年没有更好。看过表的人已判定不成立。不能量「未来谁会十倍」。 |
...
| 结论 | 三段都输每人买一点。924 那波牛里每人买一点 +76.57%，最热的只有 +24.13%。看过表的人已判定不成立。 |
...
| 结论 | 熊市相对每人买一点多 2.99 个百分点；牛市少 **40.54** 个百分点；震荡少 **24.69** 个百分点。扣费前，牛市、震荡也输。看过表的人已判定不成立。 |
...
| 用哪种回测 | `eq_us_daily`（不是 2h `event_backtest`） |
| 数据 | 无杠杆 SPY / QQQ 日线；锁到 2026-08-17；拆分红后收盘，不含分红再投 |
| IC | 没有。单品种 0/1 时钟。选股本机未重跑。 |
| 结论 | 四条「等超跌再买」或均线时钟，全期折合成一年全部低于同一段日子一直拿着。看过表的人已判定不成立。 |
...
| 分类 | beta / 市况共存 |
| 用哪种回测 | `phase1_scan_only` |
| 数据 | Epoch 季频芯片账单（CC BY）+ BTC 日线 |
| IC | 没有收益 IC。探照灯就是比例表。 |
| 结论 | 开支加速的日子，收盘在均线下的只有 13.4%；不够快的日子反而是 50.5%。方向和纸上相反。看过表的人已判定不成立。 |
