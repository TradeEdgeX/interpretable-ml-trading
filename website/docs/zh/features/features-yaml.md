...
`features.yaml` 是特征库的配方表：每个节点声明它产出哪几列、吃什么原料、怎么算。
...
- `name`：节点的名字，脚本里用它来引用。
- `outputs`：这个节点产出哪些列，存进特征库。
- `inputs`：它吃哪些原始列（这里是收盘价）。
- `params`：参数（快线 50 根、慢线 200 根）。

## 怎么找到你要的列

1. 打开 `config/strategies/<archetype>/features.yaml`。
2. 搜索列名（比如 `ema_50_200_cross_side`），找到产出它的节点。
3. 看 `inputs` 和 `params`，就知道这一列是怎么算的。
