"""Script to view and analyze the BTCUSDT backtest signals."""
...
def analyze_btcusdt_signals():
    """Analyze the BTCUSDT backtest signals."""
    # Load signals
    try:
        signals = pd.read_csv("btcusdt_backtest_signals.csv")
    except FileNotFoundError:
        print(
            "❌ btcusdt_backtest_signals.csv not found. Please run backtest_btcusdt.py first."
        )
        return
    ...
    print("BTCUSDT Signal Analysis Report")
    print("=" * 50)
    ...
    if "close" in signals.columns:
        print(
            f"Price range: ${signals['close'].min():.2f} to ${signals['close'].max():.2f}"
        )
        print(
            f"Price change: {((signals['close'].iloc[-1] / signals['close'].iloc[0]) - 1) * 100:.2f}%"
        )
    ...
    if "close" in signals.columns:
        ...
        # Mark long signals
        long_points = signals[signals["discrete_signal"] == 1]
        ...
        # Mark short signals
        short_points = signals[signals["discrete_signal"] == -1]
    ...
    plt.tight_layout()
    plt.savefig("btcusdt_signal_analysis.png", dpi=300, bbox_inches="tight")

    print(f"\n📊 Plots saved to 'btcusdt_signal_analysis.png'")
    ...
    print(
        f"\n✅ Analysis completed! Check 'btcusdt_signal_analysis.png' for visualizations."
    )


if __name__ == "__main__":
    analyze_btcusdt_signals()
