"""Create trading map visualization showing trade distribution and performance."""
...
def create_trading_map():
    """Create comprehensive trading map visualization."""
    print("🗺️ Creating Trading Map Visualization")
    print("=" * 50)
    ...
    # Add trend line
    z = np.polyfit(trades_df["duration"], trades_df["pnl"], 1)
    p = np.poly1d(z)
    ax4.plot(trades_df["duration"], p(trades_df["duration"]), "r--", alpha=0.8)
    ...
    # 8. Monthly performance heatmap (bottom-center)
    ...
    trades_df["day"] = trades_df["entry_time"].dt.day
    monthly_pnl = trades_df.groupby(["month", "day"])["pnl"].sum().unstack(fill_value=0)

    if not monthly_pnl.empty:
        sns.heatmap(monthly_pnl, annot=True, fmt=".1f", cmap="RdYlGn", center=0, ax=ax8)
        ax8.set_title("Daily P&L Heatmap", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig("trading_map_visualization.png", dpi=300, bbox_inches="tight")

    print(f"✅ Trading map saved to 'trading_map_visualization.png'")

    # Create additional detailed analysis
    create_detailed_analysis(trades_df, equity_df)


def create_detailed_analysis(trades_df, equity_df):
    """Create detailed trading analysis."""
