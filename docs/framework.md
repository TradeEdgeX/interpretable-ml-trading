...
这个框架是给 AI 助手用的工具库。技术栈保证「同一句话、同一套标准、本机可复查」。  
流程保证「人说话 → 模板过了才量 → 人写下结论」。  
各层标准保证：周期、日历、品种、数据粒度**可以换**，但不能混用别的市场的日期和比法。
...
人平时说话。没说「测试一下这个策略」，AI 只许填模板、检查齐不齐、查以前有没有判过同一句、建目录。
...
**原则：** 数学点名什么，就下什么、用什么周期。不要为了「显得科学」换一层无关的数据。缺年的段写「无样本」，不能只靠最近这段日子单独结案。
...
**通用标准（任何市场）：** 至少两段趋势 + 最近这一段（或该市场自己的熊市、牛市、消化期）。分开报五项数字。禁止合成一条折合成一年。最近这段的子窗不能单独过关。
...
1. **金叉、费率、P99、山寨最近这段** 都有「至少一段折合成一年是亏的」或最近这段更差——按各自事先写好的标准已经被数字打中。结论仍空着，等你对照标准自己写。
2. **周一** 三段折合成一年都是正的，但最近这段几乎走平；日历规律很弱，不是「日线就能当账户」。
3. **山寨 2022 没有样本** 是范围标准在工作：没有逐笔成交、还没上市，就写无样本，不拿最近这段单独过关。若改日线、从 2023 起看，那是**另一句范围**，要改模板再量。
4. **P99 必须 tick**；**周一必须日线**；**费率必须费率文件**；**十倍股必须入场日市值**；**横截面必须对等权**。同一套仓库，粒度可以完全不同。
5. **相关扫描只当探照灯。** 动量句相关是负的，和进出场成绩一致，但仍要等相对「每人买一点」的涨速结案。融资句相关接近零，按 2 小时进出场的成绩单还空着，不能下结论。
6. **换对照就是换题。** 热板块对现金可以是正收益，对同样扣过手续费的每人买一点可以不成立。SPY 等超跌再买，对现金回撤可以更浅，对同一段日子一直拿着折合成一年更低，那是保险，不是选股本事。
...
7. 怎样算不成立、五项数字，是不是按段日子事先写清楚了？比策略时账户保护关了吗？

都写进 `DECISION.md` 的「数据范围」和「验证标准」。模板过了，再让人说「测试一下这个策略」。
