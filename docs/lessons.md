...
在开盘做决定时读 **同一根已经走完的数字**，等于偷看了未来。网格对一根里面的最高最低极敏感：同一套脚本，用当根已经走完的数字，折合成一年曾经印成 50–70%；改成只读上一根已经收完的数字之后，干净那段大约 −35%～−43%。那些漂亮数字不算数。
...
能不能过关，看 `config/market_segment.yaml` 的 **熊市、牛市、最近这段** 三段同时不坏。  
最近半年那一小段是子集，**不能单独**结案。合成一段会把坏年洗掉。
