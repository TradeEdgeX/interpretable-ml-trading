...
分类先当 **拥挤回吐 / 偏均值修复**，不是选点 alpha，也不是趋势袖套。  
评测机是 **event_backtest**（BTCUSDT · 2 小时）。纸面已出表；判决留给人 `--declare`，不要手写 `verdict:`。

> 资金费率特别高就空、特别低就多，回到正常就走。费率极端拥挤的一边该回吐；熊 / 牛 / 近窗都不该把账户做穿。

这句话量的是「永续资金费率自己的拥挤」能不能在三段上都活下来。赢了原句还活着；任一段年化为负，或近窗回撤深于两个趋势段，原句死。它不是「再加一层持仓量过滤」的起点。
...
你对本仓库说同一句，并加上「测一下」。AI 必须先把原句收进[假设模板](../hypothesis_template.md)：付钱的是挤在一边的永续杠杆盘；测量对象是费率**自己的节奏**上的 z 分数（大约 8 小时一次，不是 50 根 2 小时 K 线）；先当拥挤回吐，不是趋势 alpha；任一段年化为负，或近窗回撤深于两段趋势，这句话死。模板过了、谱系没有已结案的同一句，你说了测，才下成交和费率、建 FeatureStore、走 2 小时法院。

z 分数列仓库里本来就有。缺的是「回到 0 就走」这个出口，所以引擎加了合同，而不是在回测里现场编一列——缺列补特征库，缺合同才改引擎。程序印出三段表：趋势两段小正，近窗 −3.4%，回撤也更深。两条证伪线都打中。判决留给你 `--declare`。不成立就机器不跑，也不要手盘「这回费率不一样」。不要在同一句上再拧持仓量，那是另一张纸。

这就是本仓库的哲学：启发式是先验，框架负责证伪；IC 不能上线；回测否了的句子，手做也不许。下面七段是同一条路的展开。
...
人出句（费率极端就反手，回到 0 就走）
  ...
  → 人说「测一下」才下载成交和费率、建 FeatureStore
  → 2 小时法院 event_backtest：|z| ≥ 1.5 反手，z 回 0 出场
  → 人 --declare
```

走 `mlbot research run`（这条路只派 2h `event_backtest`）。z 分数列已经在特征 DAG 里；缺的是「回到 0 就走」这个出口，所以引擎加了 `structural_exit: funding_zscore0`。缺列补特征库，缺合同才改引擎。不要在回测热路径里另写一套 `compute_*`。
...
合同用到的列必须从 FeatureStore 读（`feature_store_strict=True`）。不要在回测里现场算 z 分数。
...
尺子事先写在纸上：任一段年化 **< 0**，或近窗 MaxDD **深于**熊段和牛段，原句就死。熔断关。只报五项 KPI。
...
1. **先对窗，再比年化。** 每一段只和自己的证伪线比。不要拿熊段 +2.7% 去解释近窗 −3.4%，也不要把三段加成一条「全期还行」。
...
4. **笔数是覆盖，不是置信徽章。** 近窗 109 笔并不比熊段 128 笔「更有资格翻案」。尺子看的是年化和回撤，不是样本够不够再扫一次阈值。
...
7. **熔断关着比 edge。** 近窗亏了不能靠「要是当时熔断开着就不会亏」翻案。熔断是账户保护，不是这句话的一部分。
8. **正的趋势段不能 promote。** 证伪线写的是「任一段」。近窗已经死，整句就不能当钱用。
