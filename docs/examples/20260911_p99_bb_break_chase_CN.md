...
先当 **追涨 / 少见的大行情**：钱如果来，该来自少数很猛的一段，不是「会选点」。去掉最大几笔只用来看清类型，不能单凭这个宣判死亡。表已经出来；结不成立由你对照尺子写。
...
你对本仓库说同一句。先分类：**动量 / 肥尾右尾**。去 Top-3 只分类，不能单独用来否这条袖套——但得先真的量到右尾。粒度锁死：P99 必须从 tick 算根内最大成交额；缺月写 NaN，不许用日线冒充。模板过了、你说测了，才下载逐笔成交，才按「这一根里有没有特别大的单、收盘有没有破上轨」出表。出场写死：回到带内或满 12 根。
