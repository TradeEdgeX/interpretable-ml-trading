...
先当 **beta**：大额融资公告像一声「科技风险偏好还在」的锣，不是「会选哪一天进出」。现在只看完了日线上公告后几天的涨跌和费率；按 2 小时进出场的那张成绩单还是空的，所以还不能下结论。
...
「有某种关系」不是假设。网上的 AI 会开始讲风险偏好外溢，或者建议你去扫融资数据库。本仓库要先收成一句**能当真去做的合同**：锁定日历上的公开大额轮，公告日收盘后，次日做多 BTC，持有 5 个 UTC 日；三段都不该做穿。关系本身不上桌。

模板齐了、以前没有判过同一句，先看日线上公告后几天。这天是不是落在某笔公告后的 5 日窗里，和次日涨跌的关系是 −0.021（p = 0.40）——日线上没有「该涨」的条件。牛段窗内跑输随便拿着 BTC；近窗均值被一笔 OpenAI +8.5% 拉开，中位数是负的。AI 叙事币和 BTC 的费率一起挤（0.098），挤在一起带不来涨跌。

**故事在这里停了一拍。** 事先写的输赢线是按 2 小时进出场的全年涨速和回撤。那张成绩单还是空的。日线上再亮、再暗，都不能下结论，也不能改题去做空公告。人还没说「把 2 小时进出场跑完」，AI 就不许假装结案。不要手做「这回融资不一样」。
...
人出句（融资和 BTC 资金、涨跌有关系）
  → 先收成：公告日结束后次日做多 BTC，持有 5 个 UTC 日
  → 查过：以前有没有判过同一句
  → 先看日线上公告后几天的涨跌和费率（只照亮，不结案）
  → 按 2 小时进出场的成绩单还没跑
  → 人还不能写下结不成立
```

日线上看见相关或不相关，**不能**改题，也**不能**宣判。结案只能等 2 小时进出场的全年涨速和回撤。现在那张表是空的，所以这一页只解释日线数字怎么读。
...
| 怎样算不成立 | 任一段年化 < 0，或近窗回撤深于两个趋势段。日线上的相关或窗内均值不能当这条线。 |
...
| 数据范围 | 比特币。按 2 小时进出场（还没跑）；现在只看日线上公告后几天。币圈三段。 |
...
| `recent_range_to_bear` | 2025-01-01 → 2026-05-31 | 7 | 近窗。不能单独过关。 |
...
日线扫描只读锁死的公告日历和收盘价。真要按 2 小时进出场，同一套「今天是不是公告后」必须事先对齐好，不要算出表的时候现场另算。
...
按 2 小时进出场，还要把「今天是不是公告后」对齐好再跑（成绩单仍空）：
...
日线上**有**「这天是不是公告后」和次日涨跌同不同号的数字，但它只是探照灯，**不能结案、不能改题**。

这不是「高分名字 vs 低分名字」，而是：这天是否落在某笔公告后的 5 日窗里，和 BTC **次日**涨跌排不排在一起。n = 1611 个有标签的交易日。
...
日线扫描已经跑完。按 2 小时进出场的全年涨速和回撤 **还没跑**，所以验证只停在探照灯。

### 日线事件窗（公告次日起 5 个 UTC 日，不能结案）
...
空表的意思是：人还没有说「把 2 小时进出场跑完」。没有全年涨速和回撤，就不能下结论。
...
- **这些都不能宣判。** 事先写好的标准写的是 2h 年化和回撤。书还是空的。
...
1. **先问这张表是不是成绩单。** 日线扫描只是照亮。空着的进出场表才是事先写的输赢线。不要把窗内均值当成年化。
...
8. **人还没写下结不成立。** 结论栏空着不是「还行」。它只表示 2 小时进出场还没跑完。
