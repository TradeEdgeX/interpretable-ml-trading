...
先当 **日历规律**：说的是「周一跌了后面会涨」，不是择时大盘，也不是选股。表已经出来；结不成立由你对照尺子写。

> 周一收盘下跌后做多沪深300，持有四个交易日。用 A 股自己的三段日历测。

这句话量的是「周末坏消息在周一被定价、后四天情绪出清」能不能在 A 股三段上都活下来。赢了原句还活着；任一段年化为负，或近窗回撤深于两段趋势，原句就不成立。它不是「周一开盘买、周五收盘卖」——许可发生在**周一收盘之后**，最早周二开盘进。
...
你对本仓库说同一句。AI 先收模板：付钱的是周一的恐慌盘；周一**收盘**才知道跌了，最早周二开盘进；拿四个交易日，不是周五必须走；日历必须是 A 股自己的三段（2021 熊、924 牛、最近消化期），不许套币圈 2022。你可以对本仓库里的 AI 说「测试一下这个策略」，然后才下载沪深300日线，才按「周一收盘跌了、后四天拿着」算出一张表。

表印出来：三段年化都是很小的正数，近窗 +0.06%，赚的钱相对回撤从大约 1 掉到 0.14。事先写的两条硬门槛没有被年化打穿，但几个基点付不起当一句话来做的成本。网上的 AI 会说「还是正的」。本仓库要你对照事先写好的尺子自己写下结不成立：正的几个基点不是能当真去做的合同。回测看起来「没亏」，也不许每个周一手做同一句。

粒度跟测量对象走：这句话是日线日历，不是 2 小时逐笔。下面七段把「收盘才知道、第二天才能进」和 A 股三段怎么读写完。
...
人出句（周一跌了，后四个交易日会涨）
  ...
  → 查过：以前有没有判过同一句
  → 你说「测试一下这个策略」之后，才下载沪深300日线
  → 周一收盘跌了就做多，拿四个交易日
  → 人写下结不成立
```

这句按沪深300的**日线**进出场。日历必须用 A 股自己的三段（2021 熊、924 牛、最近消化期），不要套币圈 2022。旧的盘口辅助已经退役；下日线只为把这句量清楚，见 [RETIRED.md](../RETIRED.md)。
...
| 怎样算不成立 | 任一段年化 < 0，或近窗 MaxDD 深于两段趋势。 |
...
| `chop_recent` | 2025-06-01 → 2026-09-10 | 消化期近窗。不能单独过关。 |
...
尺子事先写在纸上：任一段年化 **< 0**，或近窗 MaxDD **深于**两段趋势，原句就不成立。
...
年化都在零附近。近窗赚的钱相对回撤，从趋势段的 1.16 / 1.52 掉到 0.14：同样浅的回撤，几乎赚不到钱。日历规律很弱，不是「日线就能当账户」。结论仍留空，等人对照尺子自己写。
...
6. **笔数 16 的 924 牛不能单独过关。** 快牛段周一下跌的次数本来就少。
