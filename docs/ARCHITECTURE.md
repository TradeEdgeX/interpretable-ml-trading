...
> 研究核：人说话 → 先填假设模板 → AI 检查模板（齐不齐、判断标准对不对）→ 只许用本机命令量 → 程序写数字 → 人写下结论。  
> 这个框架是给 AI 助手用的工具库。不是下单说明书，也不是给人手抄的命令手册。
...
本仓库把 AI **关进同一台测量机器**：它不能把网页当证据，也不能手写结论。它能做的，是把你的一句话收成能验证对错的几条，再按指令去编排下面这些命令，读命令留下的文件，用五项数字讲给你听。
...
flowchart TD
  ...
  I -->|没有| ASK{人有没有说测试一下这个策略}
  ASK -->|只要写清楚 / 建目录| INIT["mlbot research init<br/>不下载、不回测"]
  ASK -->|测试一下这个策略| MEAS[本机数据 → 特征库 → 事件回测]
...
说「测试一下这个策略」之前，AI 只许填模板、检查齐不齐、查以前有没有判过同一句、对上该用的回测、建实验目录。没让测，就不许下载、不许回测。
