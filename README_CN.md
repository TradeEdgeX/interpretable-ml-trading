...
**interpretable-ml-trading** 是一个**假设验证器**：先把想法写成模板（这是哪种社会、数学、统计现象，用什么标准判断，用哪段数据来量），再用你机器上的成交、特征库和同一把尺子算出一张表。仓库提供基础测量，欢迎你基于这些列提出更多能验证对错的想法。本仓库里的 AI 帮你把模板收齐、按格子编排命令，并用五项数字复述表；你阅读报告和策略之后，自己写下结论。
...
| 尺子 | 熊市 2022-01→2023-11、牛市 2023-06→2025-01、最近这段 2025-01→2026-05。比策略时关掉账户保护。只报折合成一年、Calmar、胜率、最大回撤、Sharpe | 不是「最近半年还行」 |
